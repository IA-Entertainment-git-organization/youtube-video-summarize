    ...
    try {
      ...
      try {
        ...
        if (durationMinutes > MAX_VIDEO_LENGTH_MINUTES) {
          ...
          throw createError(
            `Video is too long (${Math.round(durationMinutes)} minutes). Maximum allowed length is ${MAX_VIDEO_LENGTH_MINUTES} minutes.`,
        ...
        try {
          ...
          try {
            ...
            throw createError(
              `Failed to generate summary: ${(summaryError as Error).message}`,
