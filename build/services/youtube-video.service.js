...
export class YouTubeVideoService {
    async getVideoMetadata(videoUrl) {
        try {
            ...
            if (!accessCheck.accessible) {
                ...
                throw createError(`Video access check failed: ${accessCheck.reason || 'Unknown reason'}`, ErrorType.ACCESS_RESTRICTED);
            }
            ...
            throw createError(`Failed to fetch video metadata: ${error.message}`, ErrorType.API_ERROR);
        }
    }
    async getVideoTranscript(videoUrl) {
        try {
            ...
            try {
                ...
                const segments = transcriptResponse.map((item) => ({
                    text: item.text,
                ...
                try {
                    ...
                    throw createError("No transcript is available for this video using any available method.", ErrorType.NO_TRANSCRIPT);
                }
            }
        }
        catch (error) {
            ...
            throw createError(`Failed to fetch video transcript: ${error.message}. The video might not have captions available.`, ErrorType.NO_TRANSCRIPT);
        }
    }
    ...
    async tryAlternativeMetadataFetch(videoId) {
        try {
