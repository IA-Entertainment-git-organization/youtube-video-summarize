 ...
 */
export async function attemptFallbackMetadata(videoId) {
 ...
 */
export async function attemptFallbackTranscript(videoId) {
    ...
    try {
        ...
        throw createError("No transcript is available for this video or all transcript fetching methods failed.", ErrorType.NO_TRANSCRIPT);
    }
    catch (error) {
        if (error.type) {
            throw error; // Re-throw our custom errors
        }
        throw createError(`Failed to fetch transcript using all available methods: ${error.message}`, ErrorType.NO_TRANSCRIPT);
    }
}
/**
 * Utility to check if a video might be age-restricted or otherwise limited
 */
export async function checkVideoAccessibility(videoId) {
    try {
